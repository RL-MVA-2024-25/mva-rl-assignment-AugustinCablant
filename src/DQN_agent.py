...
import random 
from DQNNetwork import DQNNetwork, DQN, MLP, RNNModel
...
class DQN_AGENT():
    def __init__(self, config):
        self.env = TimeLimit(env = FastHIVPatient(domain_randomization = True), max_episode_steps = 200) 
        ...
        self.model = MLP(self.state_dim, 512, self.nb_actions, depth = 5, activation = torch.nn.SiLU(), normalization = 'None').to(self.device)
        #self.model = RNNModel(self.state_dim, 512, self.nb_actions).to(self.device)
    ...
    def train(self, path, max_episode = 200):
        episode_return = []
        episode = 0
        test_episode = 100
        best_score = 0
    ...
    def load(self, path):
        ...
        self.model = DQNNetwork(self.state_dim, self.config['nb_neurons'], self.nb_actions).to(device)
...
agent = DQN_AGENT(config4)
path = "src/models/DQN/config4_fast.pt"
episode_return = agent.train(path)

# To do :
# Look for different optimizers : Adam, RMSprop
# Clip the value of the loss gradient (-1, 1) i.e. Huber Loss -> torch.nn.SmoothL1Loss()
# Number of gradient steps after each iteration 
